...
clf = RNBA.Refridgerator(num_players = 2, num_games = 2)
...
utilities.plot(cost)
print(1-np.sum(np.abs(Y_train.T-trpredictions))/np.shape(Y_train)[0])
print(1-np.sum(np.abs(Y_dev.T-predictions))/np.shape(Y_dev)[0])
print(1-np.sum(np.abs(Y_test.T-tpredictions))/np.shape(Y_test)[0])
